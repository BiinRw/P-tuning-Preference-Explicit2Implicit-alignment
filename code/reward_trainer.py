...
import os
import deepspeed
deepspeed.ops.op_builder.CPUAdamBuilder().load() # 加载deepspeed的CPUAdam优化器

# 定义QA_trainer类，继承自transformers库的Trainer类
...
traing_args = TrainingArguments(
    output_dir = './model_output/rm-llama2-r16',
    save_strategy="epoch",
...
train_path = '../datasets/false_qa_reward.jsonl'
valid_path = '../datasets/false_qa_reward_valid.jsonl'
#test_path = ''
#model_checkpoint_path = './model_output/checkpoint-500'

train_dataset = prompt_dataset.TextDataset(train_path, tokenizer=tokenizer)
valid_dataset = prompt_dataset.TextDataset(valid_path, tokenizer=tokenizer)
